...
public class Post extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                startActivity(new Intent(Post.this, MainActivity.class).setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP));
            }
        });
        ...
        if(bdPicture!=null)
        {
            ...
            user=mBackuser;
            Picasso.get().load(mBackuser.getAvatar()).into(imAvatar);
        ...
        if(bundle!=null)
        {
            ...
            {
                Picasso.get().load(user.getAvatar()).into(imAvatar);
            }
        }
        ...
        spAlbum.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if(event.getAction() == MotionEvent.ACTION_UP)
                {
                    Intent callCreateAlbum = new Intent(getApplicationContext(),CreateAlbum.class);
                    Bundle bundle1 = new Bundle();
                    bundle1.putSerializable("User",user);
                    bundle1.putString("ND",edND.getText().toString());
                    bundle1.putString("Quyen",Access);
        ...
        imGalery.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent galery = new Intent(getApplicationContext(),pictureall.class);
    ...
    private void addData(final User user, final String ND) {
        ...
        final StorageReference mountainsRef = storageRef.child("Post/"+user.getId()+"/"+Album+"/"+calendar.getTimeInMillis()+".png");
        if(linkAnh!=null)
        {
            if(linkAnh.get(0).getByteArray()==null)
            {
                for(int i=0;i<linkAnh.size();i++)
                {
                    ...
                    final StorageReference riversRef = storageRef.child("Post/"+user.getId()+"/"+Album+file.getLastPathSegment());
            ...
            mData.child("Post").child(key).setValue(postStatus);
            mData.child("Post").child(key).child("time").setValue(ServerValue.TIMESTAMP);
        }
